...
export class LoginManager {
  ...
  start(accountId: string, input?: { provider?: string; method?: string }) {
    ...
    const run: Run = { accountId, state: emptyState() }
  ...
  private finish(loginId: string, status: LoginStatus, message: string) {
    ...
    if (status === 'succeeded') {
      const account = this.accounts.get(run.accountId)
      if (account) {
        const identity = readAccountIdentity(account.kind, account.homePath)
        this.accounts.saveIdentity(account.id, identity)
        if (
          this.accounts.labelIsAutoGenerated(account.id) &&
          (identity.email || identity.label)
        ) {
          const ordinal =
            this.accounts
              .list()
              .filter((item) => item.kind === account.kind)
              .findIndex((item) => item.id === account.id) + 1
          const kindLabel =
            account.kind[0]!.toUpperCase() + account.kind.slice(1)
          this.accounts.patch(account.id, {
            label: `${kindLabel} ${ordinal} - ${identity.email ?? identity.label}`,
          })
        }
      }
    }
