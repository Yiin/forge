...
function seed(db: DatabaseSync) {
  ...
  db.exec(`INSERT INTO sessions
    (id, project_id, harness, title, cwd, kind, status, auto_resume, created_at, last_activity_at, model)
    VALUES ('ses_1', 'prj_1', 'claude', 'Kept chat', '/tmp/forge', 'chat', 'idle', 0, 1, 2, 'opus')`)
...
describe(PROJECT_OPTIONAL, () => {
  ...
  it('leaves a fresh database with the same sessions schema as an upgrade', () => {
    const upgraded = applied(PROJECT_OPTIONAL)
    migrate(upgraded)
