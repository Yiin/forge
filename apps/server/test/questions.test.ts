...
describe('QuestionManager cancellation', () => {
  test('resolves the held question, appends cancellation, and rejects repeats', async () => {
    ...
    expect(manager.cancelQuestion(session.id, 'question-1')).toEqual({
      cancelled: true,
    })
    ...
    expect(() => manager.cancelQuestion(session.id, 'question-1')).toThrow(
      new QuestionError(409, 'Question was already answered'),
    )
    expect(() =>
      manager.answerQuestion(session.id, 'question-1', { answer: 'yes' }),
