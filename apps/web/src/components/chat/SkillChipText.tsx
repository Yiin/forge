...
export function SkillChipText({ text, skills = [] }: { text: string; skills?: string[] }) {
  ...
  for (const match of text.matchAll(SKILL_TOKEN)) {
    ...
    nodes.push(text.slice(cursor, start), <span className="skill-chip" key={`${start}-${name}`}>${name}</span>)
