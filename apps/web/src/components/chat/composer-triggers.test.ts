...
import { detectComposerTrigger, replaceComposerTrigger } from './composer-triggers'

describe('composer triggers', () => {
  it('finds a slash command at the line start, including a mid-token cursor', () => {
    expect(detectComposerTrigger('hello\n/pla now', 10)).toEqual({ kind: 'slash-command', query: 'pla', rangeStart: 6, rangeEnd: 10 })
  })
  ...
  it('detects paths and replaces only the trigger range', () => {
    ...
    expect(trigger).toEqual({ kind: 'path', query: 'src/app.ts', rangeStart: 5, rangeEnd: 16 })
    expect(replaceComposerTrigger('open @src/app.ts please', trigger!, 'src/main.ts')).toEqual({ text: 'open src/main.ts please', cursor: 16 })
  })
})
